...
def main():
    ...
    callbacks = [
        param_logger,
        FileLogger('cem_{}_log.json'.format('aaaa'), interval=STEPS_PER_EPISODE)
    ]
...
class DiscreteProcessor(Processor):
    ...
    def process_action(self, action):
        if not 0 <= action <= 2:        # Sometimes -1s come through, not sure how.
            print('WARNING: CEM action was {}, forcing to 0'.format(action))
            action = 1
        #return action - 1
        return action
...
class CEMParamLogger(Callback):
    ...
    def __init__(self, params_filename, *args, **kwargs):
        super(CEMParamLogger,self).__init__(*args, **kwargs)
